...
function toDoSubmit(event){ //toDo 입력시
    ...
    const newToDo = { //text와 id를 가지는 object
        text: toDoInput.value,
        id : Date.now(), //Date.now() : 현재 ms을 표시한다. 랜덤함수처럼 쓸 수 있음
    };
    toDos.push(newToDo); //push(VALUE) : 배열에 VALUE 저장
    toDoInput.value = ""; //값을 입력하면 빈칸으로 만든다

    saveToDos(); //todo를 localStorage에 저장
    paintTodo(newToDo);
}
...
function paintTodo(newTodo){
    ...
    li.id = newTodo.id; //Todo를 id별로 구분하기 위해 HTML에 newTodo.id를 추가한다
    //<span> : 문장 단위의 공간
    const span = document.createElement("span"); //HTML element(span)를 추가
    span.innerText = newTodo.text; //span에 새로 입력한 todo 삽입
    //<button> : 클릭하는 버튼
...
function deleteTodo(event){ //Todo 삭제
   ...
   /*toDo.id = 기존 id, li.id = 선택한 id
   toDo.id와 li.id가 같지 않으면 ture를 return하여 유지한다
   toDo.id와 li.id가 같으면 false를 return하여 제거한다
   parseInt() : STRING->NUM
   */
  saveToDos();
}
